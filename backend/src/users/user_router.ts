 ...
 * /api/users/{id}/avatar:
 *   delete:
 *     summary: Delete a user's avatar image
 *     tags: [Users]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         description: Unauthorized
 *       403:
 *         description: Forbidden - Not authorized to delete avatar for this user
 *       404:
 *         description: User not found
 */
userRouter.delete("/:id/avatar", isAuthenticated, (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    const authReq = req as AuthRequest;

    const result = userService.deleteUserAvatar(
      userId,
      authReq.payload?.user.UserId || 0,
      authReq.payload?.user.Role as UserRole
    );
 ...
 *   get:
 *     summary: Retrieve a user's enrollments
 *     tags: [Users]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         description: User not found
 */
userRouter.get("/:id/enrollments", isAuthenticated, (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    const authReq = req as AuthRequest;

    const result = userService.getUserEnrollments(
      userId,
      authReq.payload?.user.UserId || 0,
      authReq.payload?.user.Role as UserRole
    );
