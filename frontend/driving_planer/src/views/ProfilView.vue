        ...
        <div class="bg-white rounded-3xl shadow-xl border border-black/5 overflow-hidden">
          ...
          <div class="bg-black px-10 pt-12 pb-24">
            <div class="flex items-center gap-6">
              ...
              <div>
                <p class="text-white text-xl font-semibold mb-1">{{ displayName || 'Benutzername' }}</p>
          ...
          <div class="-mt-10 mx-4 mb-8 bg-white rounded-2xl shadow-md border border-black/5 px-8 py-10 flex flex-col gap-6">
            ...
            <!-- Name Field -->
            <div>
              <label class="block text-xs font-semibold uppercase tracking-widest text-black/40 mb-3">Name</label>
              <div class="relative">
            ...
            <!-- Email Field -->
            <div>
              <label class="block text-xs font-semibold uppercase tracking-widest text-black/40 mb-3">E-Mail</label>
              <div class="relative">
      ...
      <div v-if="showImageModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div class="p-8">
            ...
            >
              <i class="pi pi-cloud-upload text-3xl" :class="errors.image ? 'text-red-400' : 'text-black/20'"></i>
              <p class="text-sm font-medium" :class="errors.image ? 'text-red-500' : 'text-black/60'">Bild hierher ziehen</p>
            </div>
          ...
          <div class="px-8 py-10 flex flex-col gap-5">
            <div>
              <div class="relative">
                ...
                <input v-model="passwords.current" @input="errors.currentPass = ''" type="password" placeholder="Aktuelles Passwort" :class="[errors.currentPass ? 'border-red-500' : 'border-black/10']" class="w-full bg-black/[0.03] border rounded-2xl pl-10 pr-4 py-4 text-sm text-black focus:outline-none transition-all" />
              </div>
            ...
            <div>
              <div class="relative">
                ...
                <input v-model="passwords.new" @input="errors.newPass = ''" type="password" placeholder="Neues Passwort" :class="[errors.newPass ? 'border-red-500' : 'border-black/10']" class="w-full bg-black/[0.03] border rounded-2xl pl-10 pr-4 py-4 text-sm text-black focus:outline-none transition-all" />
              </div>
            ...
            <div>
              <div class="relative">
                ...
                <input v-model="passwords.confirm" @input="errors.confirmPass = ''" type="password" placeholder="Neues Passwort bestätigen" :class="[errors.confirmPass ? 'border-red-500' : 'border-black/10']" class="w-full bg-black/[0.03] border rounded-2xl pl-10 pr-4 py-4 text-sm text-black focus:outline-none transition-all" />
              </div>
            ...
            <button @click="closePasswordModal" class="text-black/40 text-sm py-2 hover:text-black/70 transition-colors cursor-pointer">Abbrechen</button>
          </div>
        </div>
      </div>
    </Transition>
...
const errors = reactive({
  userName: '', email: '', currentPass: '', newPass: '', confirmPass: '', image: ''
});
...
const saveProfile = () => {
  let valid = true;
  if (!userName.value) { errors.userName = "Bitte Name angeben"; valid = false; }
  if (!email.value) { errors.email = "Bitte E-Mail angeben"; valid = false; }
  if (valid) {
...
const updatePassword = () => {
  let valid = true;
  if (!passwords.current) { errors.currentPass = "Aktuelles Passwort fehlt"; valid = false; }
  if (!passwords.new) { errors.newPass = "Neues Passwort fehlt"; valid = false; }
  if (passwords.new !== passwords.confirm) { errors.confirmPass = "Passwörter nicht identisch"; valid = false; }
